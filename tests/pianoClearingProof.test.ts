...
describe('piano clearing Home proof', () => {
  ...
  it('renders the existing piano as one bounded particle cloud in a fixed scenic world', async () => {
    ...
    expect(component).toContain('const PIANO_PORTAL_SHELL_STAGES = [');
    expect(component).toContain('horizontalScale: 0.2');
    expect(component).toContain('createRefractivePianoMaterial');
    expect(component).toContain('shellLayers.map');
    expect(component).toContain("key={`portal-shell-stage-${index}`}");
    expect(component).toContain("portalSource: 'full-piano-shell'");
    expect(component).toContain('<RefractivePianoSource reducedMotion={reducedMotion} />');
    expect(component).toContain('PIANO_POSITION.y + 2.48');
