...
describe('piano clearing Home proof', () => {
  ...
  it('renders the existing piano as one bounded particle cloud in a fixed scenic world', async () => {
    ...
    expect(component).not.toContain('function pianoPortalOutlinePoint');
    expect(component).not.toContain('const pianoOutline = 1 - THREE.MathUtils.smoothstep');
    expect(component).toContain('function RefractivePianoSource');
    expect(component).toContain('function createRefractivePianoOutline');
    expect(component).toContain('new THREE.EdgesGeometry(child.geometry, 32)');
    expect(component).toContain('new THREE.LineSegments(geometry, material)');
    expect(component).toContain('function createPianoPortalTransitionGeometry');
    expect(component).toContain('const perimeterSamples = 44');
    expect(component).not.toContain('createRefractivePianoOutlineMaterial');
    expect(component).not.toContain('uLayerOpacity');
    expect(component).not.toContain('varying vec3 vLocalPosition');
    expect(component).not.toContain('shellLayers.map');
    expect(component).not.toContain('portal-shell-stage-');
    expect(component).toContain("portalSource: 'piano-edge-outline'");
    expect(component).toContain("portalTransition: 'continuous-piano-footprint-loft'");
    expect(component).toContain('<RefractivePianoSource />');
    expect(component).not.toContain('const particleGeometry = useMemo(() => {\n    const source = flowGeometry');
    expect(component).not.toContain('const particleMaterial = useMemo');
    expect(component).toContain('PIANO_POSITION.y + 2.5');
    expect(component).toContain('const canopyTravel = THREE.MathUtils.smoothstep(progress, 0.16, 1)');
