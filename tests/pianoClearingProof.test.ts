...
describe('piano clearing Home proof', () => {
  ...
  it('renders the existing piano as one bounded particle cloud in a fixed scenic world', async () => {
    ...
    expect(component).toContain('const outlineRadiusX = 2.08');
    expect(component).toContain('const outlineRadiusZ = front * 0.92 + tail * 1.5');
    ...
    expect(component).toContain('const pianoOutline = 1 - THREE.MathUtils.smoothstep(progress, 0.075, 0.34)');
    expect(component).toContain('const canopyTravel = THREE.MathUtils.smoothstep(progress, 0.34, 1)');
