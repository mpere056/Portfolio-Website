...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revision 12 strengthened LaserFlow-derived transport and introduced a first runtime budget, but remained laggy on some computers and removed too many aesthetic particles. Revision 13 restores all 700 GPU particles and targets fragment cost instead: atmosphere is merged into the field; rear/front flows, portal, observatory, city, and diagram use measured crop territories; WebGL runs at `0.9x` and 24/36 FPS; overlay particles run at `0.8x` and 24 FPS; flow noise is medium-precision and three-octave; and depth/stencil buffers are disabled. The curved composition, separated territories, stochastic tower windows, horizontal haze, and approved local attention pipeline remain. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 13 at `/projects/observatory-proof` on the computers that previously lagged, focusing on smoothness and whether restored particle density matches the preferred aesthetic. If performance still misses, profile or simplify remaining full-stage field/flow work rather than reducing particles again. If accepted, close `ART-12F` and define bounded `ART-12G` integration. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; cropped-pass east-observatory revision 13 with restored particles awaits deployed creative/performance review |
