...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revision 13 restored all 700 GPU particles while reducing fragment cost through merged/cropped passes, lower effect buffers/cadence, lighter flow noise, and disabled depth/stencil. Revision 14 adds a temporary live console for color, sizing, wisp, flow, fog, decay, and falloff values; it mutates uniforms rather than recompiling shaders and saves each reviewer's values in that browser. The curved composition, separated territories, stochastic tower windows, horizontal haze, and approved local attention pipeline remain. Creative review and a recorded chosen parameter set remain open before integration.
...
- Next exact action: Mark tunes revision 14 at `/projects/observatory-proof`, records the preferred values shown beside the controls, and checks smoothness on the computers that previously lagged. Codex then promotes those values into authored defaults and decides whether the review console remains available behind a testing affordance. If performance still misses, profile or simplify remaining field/flow work rather than reducing particles again. If accepted, close `ART-12F` and define bounded `ART-12G` integration.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east-observatory revision 14 exposes live uniform tuning so Mark can choose the final flow values before acceptance |
...
- Revision 14 converts the centralized LaserFlow constants into 13 live uniforms and adds a compact, collapsible review console with browser-local persistence and reset. Commit `9035bc9` and Production deployment `dpl_H11jwpq2VW4QhQ6dE39DKsYwYqe4` pass 207 tests, content, TypeScript, zero-error lint, and the 37-page build; public inspection confirms all controls and no console or WebGL errors.
