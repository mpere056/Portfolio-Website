...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revisions 6 and 7 restored every reference flow family but remained perceptually static to Mark. Revision 8 uses faster independent clocks, stronger shader fronts, and explicit path-bound SVG tracers so flow displacement is legible within one second. It also removes the rejected broad vertical elliptical sweep while preserving separated territories, the rotating pearl instrument, tower-only stochastic windows, 700 procedural particles, horizontal haze, and the approved local attention pipeline. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 8 at `/projects/observatory-proof`, focusing on whether the faster tracer-led motion is unmistakable but still tasteful, and confirming the rejected ellipse is gone. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east-observatory revision 8 awaits deployed creative review |
