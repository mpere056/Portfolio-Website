...
- Next exact action: deploy observatory revision 3 at `/projects/observatory-proof`, then let Mark review the decomposed scene. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
- Revision 3's focused asset/contract tests, TypeScript, and focused lint pass. Local browser inspection at `1912 x 948` confirms distinct idle and attended frames with no console warnings or errors; aggregate/build/deployment gates remain before review.
