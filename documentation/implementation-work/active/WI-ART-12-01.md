...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revisions 6 through 10 progressively removed static sources and rejected markers but still lacked unmistakable material transport. Revision 11 follows Mark's supplied LaserFlow reference more directly: triangular flow modulation, fast elongated segmented wisps, and independently advected beam fog now supplement the pressure volumes. Explicit centralized controls govern speed, strength, fog, and wisps without changing the curved composition. Separated territories, the rotating pearl instrument, tower-only stochastic windows, 700 procedural particles, horizontal haze, and the approved local attention pipeline remain. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 11 at `/projects/observatory-proof`, focusing on whether LaserFlow-style streak transport and fog advection finally make direction immediately visible. Tune the centralized controls rather than redesigning the shader if only intensity or speed remains wrong. If accepted, close `ART-12F` and define bounded `ART-12G` integration. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; LaserFlow-derived east-observatory revision 11 awaits deployed creative review |
