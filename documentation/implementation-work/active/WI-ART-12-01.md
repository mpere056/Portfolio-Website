...
The accepted correction is decomposition-first. Mark approved `1A, 2A, 3A`: a layered normal scene, lower-left ecology first, and quiet pervasive motion that becomes more expressive near attention. Structural anchors may remain spatially fixed, but every dominant region must participate through direct material movement or changing atmosphere, illumination, reflection, shadow, or occlusion. A generated transparent asset is an ingredient with a named material job, not a finished background.
...
- Next exact action: Mark reviews only `contact-sheet.png` and `proof-composite-still.png` through the short [asset review](../../art-direction/2026-07-20-museum-ambient-proof/REVIEW.md), then answers `A` approve or `B` revise.
- After approval: begin `ART-12F` with the smallest credible animated compositor proof. Do not alter Production or claim pervasive motion before `ART-12G/H`.
...
| `ART-12E` Asset review | in-review | Accepted/rejected contact sheet over diagnostic backgrounds and intended stack | Mark reviews two images; only accepted derivatives enter the compositor proof |
