...
The accepted correction is decomposition-first. Mark approved `1A, 2A, 3A`, accepted the produced material family for the proof, and requested more code-generated effects with the directional current generated procedurally. West-ecology revision 4 directly reuses the production Museum effect pipeline and is accepted. East-observatory revision 4 fixed crowding and weak no-input life; Mark approved that direction and requested a true rotating central sphere plus subtle asynchronous building windows. Revision 5 now uses procedural 3D geometry for the pearl instrument and slow stochastic per-window state transitions limited to selected skyscraper façades, while preserving separated territories, 700 procedural particles, and the approved hover pipeline. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 5 at `/projects/observatory-proof`, focusing on the sphere's volumetric rotation, slow window changes, and preserved hover behavior. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east-observatory revision 5 awaits deployed creative review |
