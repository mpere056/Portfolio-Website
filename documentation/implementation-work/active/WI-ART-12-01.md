...
- Next exact action: deploy observatory revision 4 at `/projects/observatory-proof`, then let Mark review spacing, no-input life, and preserved hover behavior. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
- Focused revision-4 asset tests, TypeScript, and lint pass. Local no-input and attended browser captures at `1280 x 720` have no console errors; aggregate/build/deployment gates remain.
