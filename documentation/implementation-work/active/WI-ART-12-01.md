...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revision 6 restored every reference flow family through procedural shader depths, but Mark found its slow distributed highlights perceptually static. Revision 7 gives the lead current coherent six-to-eight-second light fronts, a comet wake, and embedded carriers, while copper, ivory, and arch families traverse on distinct slower clocks. It preserves separated territories, the rotating pearl instrument, tower-only stochastic windows, 700 procedural particles, and the approved local attention pipeline. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 7 at `/projects/observatory-proof`, focusing on whether the traveling bands, comet wake, and curve-bound carriers make direction unmistakable without making the scene hurried. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east-observatory revision 7 awaits deployed creative review |
