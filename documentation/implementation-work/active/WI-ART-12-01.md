...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revisions 6 and 7 remained perceptually static, revision 8's circles and rods were rejected as markers, and revision 9 made the whole ribbon field too large while motion remained subtle. Revision 10 restores slender baseline strands and sends localized pressure volumes through the actual shader material, temporarily changing centerline, spread, width, aurora body, and wake on independent `2.4-7s` clocks. It preserves separated territories, the rotating pearl instrument, tower-only stochastic windows, 700 procedural particles, horizontal haze, and the approved local attention pipeline. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 10 at `/projects/observatory-proof`, focusing on whether the localized expanding volumes visibly travel while most of each ribbon stays slender. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; pressure-flow east-observatory revision 10 awaits deployed creative review |
