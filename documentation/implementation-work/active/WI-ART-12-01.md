...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revisions 6 through 11 progressively removed static sources and rejected markers, then adopted LaserFlow-derived modulation, wisps, and beam fog, but movement remained too subtle and the page lagged on some computers. Revision 12 raises visible transport to `2.15x` speed and `1.24` strength while bounding runtime work at `1x` DPR and 30/45 FPS. Fog is three-octave, particle translation is GPU-owned, the three depths total 420 points, the orb uses detail 3, the overlay canvas is capped at 30 FPS, and pointer state is coalesced. The curved composition, separated territories, stochastic tower windows, horizontal haze, and approved local attention pipeline remain. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 12 at `/projects/observatory-proof`, focusing on whether movement is now immediately legible and whether the page is materially smoother on the computers that previously lagged. Tune centralized controls or performance limits rather than redesigning the composition if either dimension still misses. If accepted, close `ART-12F` and define bounded `ART-12G` integration. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; faster, bounded east-observatory revision 12 awaits deployed creative/performance review |
