...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revision 17 proved source-faithful LaserFlow semantics and repaired the live DOM-to-GPU control bridge; Mark accepted that behavior. Revision 18 composed six cropped native LaserFlow families but made broad native beams visually dominant. Revision 19 preserves their native controls and dynamics while shaping the output into the earlier woven-current language: three to ten narrow procedural filaments per family, independently traveling pressure bodies, bounded shimmer, route-specific convergence, and no broad spotlight-dominant output. The older front/rear shaders remain dormant. Browser-local per-family tuning, 700 particles, separated structural territories, stochastic tower windows, the 3D orb, one-screen framing, and the approved Museum attention pipeline remain.
...
- Next exact action: Mark reviews live revision 19 at `/projects/observatory-proof`, compares the braided current silhouette with the earlier accepted appearance, and checks the route on the previously affected computers. Adjust only named family optical presets through the selector; do not alter native control meanings. If visual and performance review pass, accept east-observatory `ART-12F` and begin `ART-12G` Museum integration.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east revision 19 awaits braided-composition and affected-computer review |
