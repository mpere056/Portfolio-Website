...
- Next exact action: let Mark review deployed observatory revision 2 at `/projects/observatory-proof`. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; the distinct east-observatory extension awaits deployed creative review |
