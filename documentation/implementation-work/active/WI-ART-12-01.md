...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revision 17 proved source-faithful LaserFlow semantics and repaired the live DOM-to-GPU control bridge; Mark accepted that behavior. Revision 19 preserves those native controls while restoring the earlier woven-current language. Revision 20 records Mark's selected vertical sizing, speed, strength, decay, and falloff values for all six families, advances saved presets to `v5`, and replaces endpoint-wide hover with narrow route-segment attention plus a local shader field. Browser-local per-family tuning, 700 particles, separated structural territories, stochastic tower windows, the 3D orb, one-screen framing, and the approved Museum attention pipeline remain.
...
- Next exact action: Mark reviews live revision 20 at `/projects/observatory-proof`, confirms the selected defaults survive a fresh visit, checks that hover energizes only the nearby current, and profiles the route on the previously affected computers. If visual and performance review pass, accept east-observatory `ART-12F` and begin `ART-12G` Museum integration.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; east revision 20 awaits selected-preset, localized-hover, and affected-computer review |
