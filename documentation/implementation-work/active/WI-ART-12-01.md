...
The accepted correction is decomposition-first. West-ecology revision 4 is accepted. East-observatory revisions 6 and 7 remained perceptually static, while revision 8's explicit circles and rods were rejected as markers rather than flow. Revision 9 removes that overlay and animates the actual shader ribbons through coherent moving curvature, thickness, crossings, texture, and luminosity on independent `2.4-7s` clocks. It preserves separated territories, the rotating pearl instrument, tower-only stochastic windows, 700 procedural particles, horizontal haze, and the approved local attention pipeline. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 9 at `/projects/observatory-proof`, focusing only on whether the marker-free copper and cyan-gold ribbons visibly flow at full-page scale. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; marker-free east-observatory revision 9 awaits deployed creative review |
