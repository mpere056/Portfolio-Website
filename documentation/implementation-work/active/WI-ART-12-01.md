...
The accepted correction is decomposition-first. Mark approved `1A, 2A, 3A`, accepted the produced material family for the proof, and requested more code-generated effects with the directional current generated procedurally. West-ecology revision 4 directly reuses the production Museum effect pipeline and is accepted. East-observatory revision 3 proved decomposed plates but Mark found its masses too crowded and its no-input material life too quiet. Revision 4 now separates portal, lattice, and city territories; replaces every ambient raster particle with 700 procedural points; strengthens independently timed idle material light and optical motion; and preserves the hover effects Mark explicitly approved. Creative review remains open before integration.
...
- Next exact action: let Mark review deployed observatory revision 4 at `/projects/observatory-proof`, focusing on spacing, no-input life, and preserved hover behavior. If accepted, close `ART-12F` and define bounded `ART-12G` integration from both accepted proofs. Technical documents are optional.
...
| `ART-12F` Compositor | in-review | Hybrid one-clock WebGL proofs with plate shaders and procedural systems | West ecology revision 4 is accepted; less-crowded east-observatory revision 4 awaits deployed creative review |
