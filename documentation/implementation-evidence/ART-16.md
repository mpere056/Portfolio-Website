...
| Scope | Camera advances farther right along the near meadow and aims across the valley from a restrained low-grazing angle, with a final slight left yaw toward the opposite grass hill. The piano moves to a high, river-safe right-bank coordinate and projects near horizontal center at roughly one quarter of the viewport above the bottom. Its sampled GLTF bounds now recenter the model and align its true lowest vertex to the terrain sample, replacing the former guessed vertical offset. Saturated blue/gold points are replaced by smaller pearl and cool-white particles over the existing translucent silhouette. The artificial piano grass-darkening patch is removed; ground and river meshes extend through the distant valley; the river fades before its terminal edge; wider, denser ridge bands keep oblique side edges outside the frustum; and the existing grass distribution extends from world `z=11` to `z=24` so the camera-facing terrain edge remains covered. Ambient blade wind is reduced to `0.52`. Cursor motion is projected onto the terrain, its hit point follows with `8.5` damping, movement energy decays through a `2.7` impulse envelope and `7.5` influence follower, direction turns more gradually, and the softer `4.2` world-unit falloff removes the former abrupt wake boundary. Camera pointer travel increases from `0.08` to a still-bounded `0.18`. |
| Runtime policy | One existing canvas; unchanged `9,000`-point piano and `220,000`-blade meadow budgets; DPR capped at `1.25`; one cached model-bounds calculation; cursor response uses preallocated vectors and shader uniforms only; no new draw systems, post-processing, shadows, or physics. |
| Automated evidence | All 57 test files / 240 tests, TypeScript, focused ESLint, content validation, and the clean 40-route Production build pass. Tests bind the farther-right camera, river-safe high-meadow piano coordinate, geometry-aware grounding, non-saturated particle treatment, continuous valley geometry, hidden ridge edges, tapered river endpoint, foreground grass extent, quieter wind, and terrain-local cursor response. |
| Browser evidence | Production `1080 x 894` review confirms the piano is lower-center on continuous foreground grass through the bottom viewport edge, the bridge remains a separate middle-distance cue, distant ridge edges and the bright valley wedge are absent, and the former blue/gold dots are replaced by fine pale light particles. Pointer sweeps across the foreground exercise the eased local wake and wider camera breathing without browser warnings or errors. |
| Production evidence | Commits `48ef8ba`, `5718cd8`, `223d226`, `f008907`, and `406634a` are pushed to `main` and live through Git-connected Production at `/home-world-proof`; the public marker reports `data-grass-wind="0.52"`, `data-grass-cursor="terrain-local"`, and the unchanged `220000-grass/9000-piano-points/no-post` budget. |
