...
| Scope | Camera advances farther right along the near meadow and aims across the valley from a restrained low-grazing angle, with a final slight left yaw toward the opposite grass hill. The piano moves to a high, river-safe right-bank coordinate and projects near horizontal center at roughly one quarter of the viewport above the bottom. Its sampled GLTF bounds now recenter the model and align its true lowest vertex to the terrain sample, replacing the former guessed vertical offset. Saturated blue/gold points are replaced by smaller pearl and cool-white particles over the existing translucent silhouette. The artificial piano grass-darkening patch is removed; ground and river meshes extend through the distant valley; the river fades before its terminal edge; wider, denser ridge bands keep oblique side edges outside the frustum; and the existing grass distribution extends from world `z=11` to `z=24` so the camera-facing terrain edge remains covered. Ambient blade wind is reduced to `0.52`. Cursor motion is projected onto the terrain, its hit point follows with `8.5` damping, movement energy decays through a `2.7` impulse envelope and `7.5` influence follower, direction turns more gradually, and the local wake contracts from `4.2` to `3.2` world units. Camera pointer travel increases from `0.08` to a still-bounded `0.18`. The train is removed while the viaduct remains as a middle-distance depth cue; seven distant birds glide and flap independently across the open sky. |
| Runtime policy | One existing canvas; unchanged `9,000`-point piano and `220,000`-blade meadow budgets; DPR capped at `1.25`; one cached model-bounds calculation; cursor response uses preallocated vectors and shader uniforms only; seven birds share one instanced draw and preallocated transform helper; no post-processing, shadows, or physics. |
| Automated evidence | All 57 test files / 240 tests, TypeScript, focused ESLint, content validation, and the clean 40-route Production build pass. Tests bind the farther-right camera, river-safe high-meadow piano coordinate, geometry-aware grounding, non-saturated particle treatment, continuous valley geometry, hidden ridge edges, tapered river endpoint, foreground grass extent, quieter wind, `3.2`-unit terrain-local cursor response, train absence, and seven-bird single-instanced-render contract. |
| Browser evidence | Production `1080 x 894` review confirms the piano is lower-center on continuous foreground grass through the bottom viewport edge, the viaduct is empty, seven restrained distant silhouettes remain readable against the pale sky, and the browser emits no warnings or errors. |
...
| Scope | Commit `6477e5c` replaces the empty staging room with 10 authored slides grounded in the reviewed About timeline. The current revision narrows those slides to Mark's programming and career story: LittleBigPlanet 2 logic, MW2 lobby operation, C++, AoTTG modding, Discord scale, CRA, degree completion and AirOps, Dreamlife, and the current FirePower Capital software role, followed by an AI-in-business handoff. The deck uses sparse copy, distinct CSS/SVG visual instruments, direct progress controls, previous/next controls, keyboard navigation, swipe, fullscreen support, reduced motion, and a Home return. It suppresses the global tour, discovery prompt, semantic signal, archive AI, and site audio only on `/presentation`. |
