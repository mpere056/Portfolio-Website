...
## EV-ART-11-03: Production Visual Acceptance Failure

Status: rejected as creative acceptance; functional rollout evidence retained.

Mark's 2026-07-20 production screenshots showed that `/projects` required vertical scrolling to move from an oversized introduction into the complete nine-project field. A direct browser audit also showed that Dreamlife, LifeInbox, and Sudoku Together rendered the approved static material but not a visibly distinct dynamic scene on their direct landing surfaces. This invalidates the creative-acceptance implication of `EV-ART-11-02`; it does not invalidate its route, deployment, fallback, or lifecycle evidence.
...
Local browser evidence at 1912x948:

- document scroll height and client height are both `948` in the Museum lobby;
- all nine signal hit regions are inside the viewport with zero pairwise overlaps;
- Dreamlife hover moves the Museum aperture to its reviewed signal and raises the response strengths;
