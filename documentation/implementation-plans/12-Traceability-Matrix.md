...
| Phase 4E ambient Museum and flagships | current: `ART-12` ready, then `ART-13`; earlier `ART-07` through `ART-11` are paused interaction foundations | Museum passes ordered `A-H` gates; production assets start only after mapping/briefing; LifeInbox, Dreamlife, and Sudoku then pass separately before the next route produces assets |
