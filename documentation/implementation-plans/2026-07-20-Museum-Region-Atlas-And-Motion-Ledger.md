...
| `MUS-R08` | Rings/apertures rotate around credible pivots; glass catches changing light | Cyan stream and east haze cross through lattice depth | Sudoku focus addresses one lattice path | Split mechanism plates, pivots, depth/occlusion/refraction/emission maps | Representative lattice plate and runtime proof exist; production integration missing |
| `MUS-R09` | Waveform travels; distant lights vary asymmetrically | Far haze passes between spires | East historical signal increases local legibility | Far city plate, SVG/texture waveform, atmosphere and emission masks | Representative city plate and asymmetric lighting proof exist; production integration missing |
...
| `MUS-R13` | Internal aperture rotates/settles and portal light changes | Lower current passes behind; near mist passes in front | Discord bot focus tunes aperture and reveals reviewed evidence | Fixed device plate, aperture plate/pivot, portal emission, depth mask | Representative portal plate and refraction proof exist; production integration missing |
| `MUS-R14` | Crystal/interior illumination migrates; edge shadows change | East haze and rare foreground matter cross architecture | Game-mod focus raises local crystal signal | Fixed architecture plate, emission/gobo masks, atmosphere/occluders | Representative city plate and illumination proof exist; production integration missing |
...
After Mark accepted revision 4's west-ecology direction, `ART-12F` opened one deliberately different proof around `MUS-R08`, supported by adjacent `MUS-R05`, `MUS-R06`, `MUS-R09`, `MUS-R13`, and `MUS-R14`. The route is `/projects/observatory-proof`. It tests fixed architecture with unequal optical pivots, local lens refraction, directional signal transport, changing diagram geometry, layered haze, code-generated depth matter, migrating illumination, and the exact production Museum attention-effects stack.

Revision 3 advances this from a crop-based extension to a bounded decomposed compositor: particle-free field, lattice, city, and portal plates have separate shader jobs, while all particles, currents, fog, wavefronts, and attention effects are generated in code. A fifth particle-free composite remains fallback-only. These plates prove the production method but do not authorize whole-east-side integration before creative review and `ART-12G` planning.
