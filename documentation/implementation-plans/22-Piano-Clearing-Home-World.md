...
| `E2` Music bounded material proof | in-progress | One private territory beside the piano has shared material, local response, adaptive calm/reduced behavior, and a neutral failure fallback; measured cost and review remain | `ML0-ML6` pass before any full selected-world expansion |
...
- Exact next action: capture Plan `24` neutral/proof renderer metrics and slower-device evidence for the `ML4/ML5` candidate. Stop at `ML6` before selected-world expansion; do not start hover, Life, or Play runtime work yet.
