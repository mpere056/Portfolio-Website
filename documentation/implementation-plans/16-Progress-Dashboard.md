...
The original rollout proved route health, fallbacks, lifecycle policy, and renderer safety, while Production `fce50af` fixed the scrolling Museum and direct interaction. `ART-12A-E` preserve/map the baseline and produce an accepted lower-left material stack. West revision 4 is accepted. East revision 14 retains revision 13's 700-particle cropped runtime and adds live uniform controls for creative tuning; rejected markers and the vertical ellipse are absent. Full-Museum integration and `V-33` acceptance are still not claimed.
...
| `in-review` | 1 | `ART-12F` asks Mark to observe the real animated compositor before Museum integration |
...
| 51 | `ART-12` | Ambient contract, decomposition, and Museum proof | in-review | `ART-12A-E` complete; east revision 14 exposes 13 live shader values over the accepted cropped, dense runtime | Mark records preferred values and reviews affected-computer smoothness; acceptance completes compositor selection and permits bounded `12G` integration |
...
| --- | --- | --- | --- | --- | --- | --- |
| Now | `WI-ART-12-01` | in-review | `ART-12` | West revision 4 accepted; live-tunable east revision 14 is live at `9035bc9` | Mark chooses and records flow values and reviews smoothness; after acceptance complete `12F` and begin `12G` | 2026-07-22 |
...
| --- | --- | --- | --- | --- | --- |
| `WI-ART-12-01` | Which revision-14 flow values best balance clearly visible movement, tasteful scale, and smooth performance on affected computers? | Mark | `/projects/observatory-proof`; `9035bc9`; 13 live controls over 700 particles and cropped shader coverage; 207 tests and build pass | 2026-07-22 | Record the displayed values before `ART-12G` changes the Museum |
...
| --- | --- | --- | --- |
| 2026-07-22 | Added a temporary live observatory tuning console | `ART-12F`, `CAP-ART-014`, `WI-ART-12-01` | Commit `9035bc9` and deployment `dpl_H11jwpq2VW4QhQ6dE39DKsYwYqe4` expose color plus 12 shader parameters without material recompilation; values persist locally until reset |
