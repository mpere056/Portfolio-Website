...
The original rollout proved route health, fallbacks, lifecycle policy, and renderer safety, while Production `fce50af` fixed the scrolling Museum and direct interaction. `ART-12A-E` preserve/map the baseline and produce an accepted lower-left material stack. West revision 4 is accepted. East revision 15 retains the 700-particle cropped runtime and provides visually verified authoritative controls over both procedural flow planes; rejected markers and the vertical ellipse are absent. Full-Museum integration and `V-33` acceptance are still not claimed.
...
| --- | --- | --- | --- | --- | --- | --- |
| Now | `WI-ART-12-01` | in-review | `ART-12` | West revision 4 accepted; authoritative live-tunable east revision 15 is live at `ac6d742` | Mark chooses and records flow values and reviews smoothness; after acceptance complete `12F` and begin `12G` | 2026-07-22 |
