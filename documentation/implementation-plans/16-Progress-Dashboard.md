...
| `CAP-ART-013/014` | `WI-ART-12-01` | `12A-E` pass; west proof accepted; east proof has decomposed ribbon-free plates, a native-semantics LaserFlow calibration plane, atmosphere/particles, rotating 3D geometry, stochastic tower light, fallback, and focused checks | Mark's control-semantics review, route-specific east composition, and `12G/H` full-Museum integration/release | Preserve Production `fce50af`; keep proofs off the main Museum until accepted | Review `/projects/observatory-proof`; compose only after the native controls pass | Representative runtime evidence exists; no full `V-33` evidence yet |
