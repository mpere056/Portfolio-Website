...
| `CAP-ART-013/014` | `WI-ART-12-01` | `12A-E` pass; west proof accepted; east proof has six cropped native LaserFlow families, decomposed ribbon-free plates, atmosphere/particles, rotating 3D geometry, stochastic tower light, fallback, and focused checks | Mark's composed-scene and affected-computer review, then `12G/H` full-Museum integration/release | Preserve Production `fce50af`; keep proofs off the main Museum until accepted | Release revision 18 and review `/projects/observatory-proof` on both smooth and previously laggy computers | Representative runtime evidence exists; no full `V-33` evidence yet |
...
| --- | --- | --- | --- |
| 2026-07-23 | Composed the accepted native LaserFlow behavior into the east observatory | `ART-12F`, `CAP-ART-014`, `WI-ART-12-01` | Six cropped, independently phased current families now preserve native equations and slider meanings while adding route-owned layout, local attention, feathered edges, and per-family persistence; local idle and attended visual checks pass |
