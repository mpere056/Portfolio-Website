...
| --- | --- |
| Plan ID | `HOME-WORLD` |
| Status | Active; architecture approved, implementation ready |
...
| `B` Taxonomy | `KG-07` | Practice nodes, project classification, graph queries, validators | Every project has one primary practice |
| `C` Contracts | `ARC-06`, `EXP-08` | Territory IDs, pure attention reducer, hysteresis, route/restore policy | Deterministic tests cover neutral, local attention, selection, Back, and reduced motion |
...
- Known-good implementation: `68e0897` on `main`; current public Home and project routes remain unchanged.
...
- Current gap: no practice taxonomy, shared attention reducer, neutral five-anchor shell, or route migration exists yet.
- Next exact action: implement Stage `B/C` together as a non-visual foundation: practice IDs/classification plus a pure tested attention-state contract. Do not alter public routing in that change.
