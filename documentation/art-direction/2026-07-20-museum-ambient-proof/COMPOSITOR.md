...
Stage: `ART-12F`
State: revision 3 implemented locally; awaiting redeploy and Mark review
...
Mark rejected revision 1 because the dominant coral still read as a static poster, clustered systems answered the pointer together, and the accepted Museum's richer lighting language had been lost. Revision 2 established credible quiet idle life. Mark then asked for stronger scene integration at the organism root, a clearly intensified attended state, and the original Museum aperture/distortion language. Revision 3 keeps the accepted idle behavior while adding organic root dissolution, crossing foreground fog, one normalized stage attention coordinate, object-local acceleration profiles, and a code-generated aperture/diffraction membrane shared by CSS and WebGL effects.
...
| Illumination | Independent amber/cyan passages and caustic pools at idle; pointer adds a localized refraction lens, radial aperture, diffraction mesh, and material-specific intensity without changing the global clock |
...
- Focused compositor/material/policy tests pass `11/11`; the aggregate suite passes `200/200`, TypeScript passes, content validation passes, and lint reports zero errors.
...
- Atmosphere, particles, and occluders ignore pointer input. Coral, organism, rings, and current use different local radii, lag, attack, and release values; the aperture follows the shared stage coordinate independently.
