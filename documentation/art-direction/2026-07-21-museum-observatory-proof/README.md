...
Revision 13 retains revision 12's stronger marker-free LaserFlow transport while restoring all 700 GPU particles. The performance strategy moves away from sacrificing particle density: atmosphere is merged into the field, five transparent/procedural layers render only inside measured authored territories, flow-body noise uses three octaves at medium precision, and depth/stencil buffers are disabled. Moving pressure volumes, segmented wisps, and beam-masked fog remain distinct from the slender baseline strands. Rejected markers and the vertical ellipse remain absent.
