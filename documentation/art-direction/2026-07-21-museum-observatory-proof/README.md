...
Revision 8 retains the fully procedural flow stack and makes direction unmistakable at normal viewing distance. The cyan-gold lead now crosses in about `2.8-3.6s`; copper, ivory, and arch families use independent `6.5s`, `8.5s`, and `11s` clocks. Shader fronts and carriers are reinforced by an SVG tracer layer with long moving luminous segments and distinct signal heads. The broad vertical elliptical light sweep rejected by Mark has been removed; horizontal haze, particles, local refraction, the rotating pearl instrument, tower-only stochastic windows, and the accepted Museum attention stack remain intact.
