...
Revision 10 keeps revision 9's marker-free shader ownership but restores the smaller baseline ribbon scale. Localized wrapped pressure volumes now travel through each family like liquid through a hose: they temporarily displace the centerline, spread the strands, widen the cores, brighten a soft aurora body, and leave an asymmetric wake before the ribbon contracts. The cyan-gold lead crosses in `2.4s`; copper, ivory, and arch families use independent `4.2s`, `5.4s`, and `7s` clocks. The rejected markers and vertical elliptical sweep remain absent; horizontal haze, particles, local refraction, the rotating pearl instrument, tower-only stochastic windows, and the accepted Museum attention stack remain intact.
