...
Revision 9 removes revision 8's rejected SVG tracer paths, circles, rods, and flow-head treatment. Motion now belongs entirely to the procedural ribbon material: broad coherent crests change curvature, width, crossings, internal texture, and luminosity while smaller strands retain independent turbulence. The cyan-gold lead crosses in `2.4s`; copper, ivory, and arch families use independent `4.2s`, `5.4s`, and `7s` clocks. The rejected vertical elliptical sweep remains absent; horizontal haze, particles, local refraction, the rotating pearl instrument, tower-only stochastic windows, and the accepted Museum attention stack remain intact.
