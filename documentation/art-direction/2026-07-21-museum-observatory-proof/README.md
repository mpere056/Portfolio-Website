...
Revision 12 retains the marker-free LaserFlow-derived ribbons while making transport more explicit and cheaper to render. The centralized flow controls now use a `2.15x` speed multiplier, `1.24` strength, `1.08` wisp speed at `1.92` intensity, and `0.66` fog advection. Moving pressure volumes, segmented wisps, and beam-masked fog remain distinct from the slender baseline strands. Rejected markers and the vertical ellipse remain absent.

The structures themselves do not wobble. Reduced motion removes WebGL and preserves the particle-free fallback. Hidden tabs stop the render loop. Visible rendering uses one device pixel per CSS pixel and demand scheduling at 30 FPS idle or 45 FPS under attention. Particle travel is vertex-shader owned, the three particle depths total 420 points, flow fog uses three octaves, and the small orb uses detail level 3. The proof remains one viewport and is isolated from `/projects` and `/projects/ambient-proof`.
